...
Spring AI 支持 OpenAI 的审核模型，该模型允许您检测文本中潜在的有害或敏感内容。
访问此 https://platform.openai.com/docs/guides/moderation[指南] 获取有关 OpenAI 审核模型的更多信息。

== 前提条件

. 创建 OpenAI 账户并获取 API 密钥。您可以在 https://platform.openai.com/signup[OpenAI 注册页面] 注册，并在 https://platform.openai.com/account/api-keys[API 密钥页面] 生成 API 密钥。
. 将 `spring-ai-openai` 依赖添加到项目的构建文件中。更多信息，请参考 xref:getting-started.adoc#dependency-management[依赖管理] 部分。
...
Spring AI 为 OpenAI 审核模型提供了 Spring Boot 自动配置。
...
[NOTE]
====
嵌入自动配置的启用和禁用现在通过前缀为 `spring.ai.model.moderation` 的顶级属性进行配置。

要启用，spring.ai.model.moderation=openai（默认启用）

要禁用，spring.ai.model.moderation=none（或任何不匹配 openai 的值）
...
| spring.ai.openai.moderation.base-url   | 连接 URL |  https://api.openai.com
| spring.ai.openai.moderation.api-key    | API 密钥 |  -
| spring.ai.openai.moderation.organization-id | 可选，您可以指定用于 API 请求的组织 |  -
| spring.ai.openai.moderation.project-id      | 可选，您可以指定用于 API 请求的项目 |  -
| spring.ai.openai.moderation.options.model  | 用于审核的模型 ID | text-moderation-latest
|====
...
如果设置了 `spring.ai.openai.moderation.base-url`、`spring.ai.openai.moderation.api-key`、`spring.ai.openai.moderation.organization-id` 和 `spring.ai.openai.moderation.project-id` 属性，它们将优先于通用属性。
这对于想要为不同模型和不同模型端点使用不同 OpenAI 账户的情况很有用。

提示：所有以 `spring.ai.openai.moderation.options` 为前缀的属性都可以在运行时被覆盖。

== 运行时选项
OpenAiModerationOptions 类提供了在进行审核请求时要使用的选项。
在启动时，使用由 spring.ai.openai.moderation 指定的选项，但您可以在运行时覆盖这些选项。
...
== 示例代码
`OpenAiModerationModelIT` 测试提供了一些关于如何使用该库的通用示例。您可以参考此测试获取更详细的使用示例。
