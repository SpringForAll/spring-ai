...
注意：截至 2024年2月5日，旧的 `OutputParser`、`BeanOutputParser`、`ListOutputParser` 和 `MapOutputParser` 类已被弃用，取而代之的是新的 `StructuredOutputConverter`、`BeanOutputConverter`、`ListOutputConverter` 和 `MapOutputConverter` 实现。
后者是前者的直接替代品，提供相同的功能。变更的主要原因是为了命名，因为实际上并没有进行任何解析操作，同时也与 Spring 的 `org.springframework.core.convert.converter` 包保持一致，带来了一些改进的功能。

LLM 生成结构化输出的能力对于依赖可靠解析输出值的下游应用程序来说非常重要。
...
如下图所示，这种方法围绕 LLM 文本补全端点运行：

image::structured-output-architecture.jpg[结构化输出转换器架构, width=900, align="center"]

使用通用补全 API 从大型语言模型 (LLM) 生成结构化输出需要仔细处理输入和输出。结构化输出转换器在 LLM 调用前后都扮演着关键角色，确保实现所需的输出结构。
...
提示：`StructuredOutputConverter` 是尽最大努力将模型输出转换为结构化输出。
AI 模型不能保证按照要求返回结构化输出。
模型可能不理解提示或无法按要求生成结构化输出。
考虑实现验证机制以确保模型输出符合预期。

提示：`StructuredOutputConverter` 不用于 LLM xref:api/tools.adoc[工具调用]，因为此功能默认就提供结构化输出。
...
它结合了 Spring https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/core/convert/converter/Converter.html[Converter<String, T>] 接口和 `FormatProvider` 接口
...
image::structured-output-api.jpg[结构化输出 API, width=900, align="center"]
...
格式指令通常使用 xref:api/prompt.adoc#_prompttemplate[PromptTemplate] 附加到用户输入的末尾，如下所示：
...
image::structured-output-hierarchy4.jpg[结构化输出类层次结构, width=900, align="center"]

* `AbstractConversionServiceOutputConverter<T>` - 提供预配置的 link:https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/core/convert/support/GenericConversionService.html[GenericConversionService] 用于将 LLM 输出转换为所需格式。不提供默认的 `FormatProvider` 实现。
* `AbstractMessageOutputConverter<T>` - 提供预配置的 https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/jms/support/converter/MessageConverter.html[MessageConverter] 用于将 LLM 输出转换为所需格式。不提供默认的 `FormatProvider` 实现。
* `BeanOutputConverter<T>` - 配置指定的 Java 类（例如 Bean）或 link:https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/core/ParameterizedTypeReference.html[ParameterizedTypeReference]，此转换器使用 `FormatProvider` 实现，指导 AI 模型生成符合从指定 Java 类派生的 `DRAFT_2020_12`、`JSON Schema` 的 JSON 响应。随后，它使用 `ObjectMapper` 将 JSON 输出反序列化为目标类的 Java 对象实例。
...
此注解允许您指定属性在模式中出现的确切顺序，而不考虑它们在类或记录中的声明顺序。
...
* xref:api/chat/openai-chat.adoc#_structured_outputs[OpenAI 结构化输出] 可以确保您的模型生成严格符合您提供的 JSON Schema 的响应。您可以选择 `JSON_OBJECT`，它保证模型生成的消息是有效的 JSON，或者选择 `JSON_SCHEMA` 并提供模式，保证模型将生成与您提供的模式匹配的响应（`spring.ai.openai.chat.options.responseFormat` 选项）。
* xref:api/chat/azure-openai-chat.adoc[Azure OpenAI] - 提供 `spring.ai.azure.openai.chat.options.responseFormat` 选项，指定模型必须输出的格式。设置为 `{ "type": "json_object" }` 启用 JSON 模式，它保证模型生成的消息是有效的 JSON。
* xref:api/chat/ollama-chat.adoc[Ollama] - 提供 `spring.ai.ollama.chat.options.format` 选项来指定返回响应的格式。目前，唯一接受的值是 `json`。
* xref:api/chat/mistralai-chat.adoc[Mistral AI] - 提供 `spring.ai.mistralai.chat.options.responseFormat` 选项来指定返回响应的格式。设置为 `{ "type": "json_object" }` 启用 JSON 模式，它保证模型生成的消息是有效的 JSON。
