...
[NOTE]
====
关于 `ChatClient` 中命令式和响应式编程模型组合使用的相关说明，请参阅本文档底部的 xref:api/chatclient.adoc#_implementation_notes[实现说明] 部分。
====

流畅的 API 提供了构建 xref:api/prompt.adoc#_prompt[Prompt] 各个组成部分的方法，这些 Prompt 将作为输入传递给 AI 模型。
...
您可以为任何 xref:api/chatmodel.adoc[ChatModel] Spring Boot 自动配置获取一个自动配置的 `ChatClient.Builder` 实例，或者以编程方式创建一个。

=== 使用自动配置的 ChatClient.Builder

在最简单的用例中，Spring AI 提供了 Spring Boot 自动配置，为您创建一个原型 `ChatClient.Builder` bean，您可以将其注入到您的类中。
...
AI 模型的响应是一个由 `xref:api/chatmodel.adoc#ChatResponse[ChatResponse]` 类型定义的丰富结构。
它包括有关如何生成响应的元数据，还可以包含多个响应，称为 xref:api/chatmodel.adoc#Generation[Generation]s，每个都有自己的元数据。
...
同时，您应该使用 xref:api/structured-output-converter.adoc#StructuredOutputConverter[结构化输出转换器] 显式转换聚合响应，如下所示。
...
默认情况下，Spring AI 使用 `StTemplateRenderer` 实现，它基于 Terence Parr 开发的开源 https://www.stringtemplate.org/[StringTemplate] 引擎。

Spring AI 还提供了 `NoOpTemplateRenderer` 用于不需要模板处理的情况。

注意：直接在 `ChatClient` 上配置的 `TemplateRenderer`（通过 `.templateRenderer()`）仅适用于直接在 `ChatClient` 构建器链中定义的提示内容（例如，通过 `.user()`、`.system()`）。
它不会影响 xref:api/retrieval-augmented-generation.adoc#_questionansweradvisor[Advisors] 内部使用的模板，如 `QuestionAnswerAdvisor`，它们有自己的模板自定义机制（参见 xref:api/retrieval-augmented-generation.adoc#_custom_template[自定义 Advisor 模板]）。
...
xref:api/advisors.adoc[Advisors API] 提供了一种灵活而强大的方式来拦截、修改和增强 Spring 应用程序中的 AI 驱动交互。
...
请参阅 xref:ROOT:api/retrieval-augmented-generation.adoc[检索增强生成] 指南。
...
提示：Spring AI 支持 LLM 和向量存储交互的可观察性。有关更多信息，请参阅 xref:observability/index.adoc[可观察性] 指南。
...
有关更多详细信息和用法示例，请参阅 xref:api/chat-memory.adoc[聊天内存] 文档。

== 实现说明

`ChatClient` 中命令式和响应式编程模型的组合使用是 API 的一个独特方面。
