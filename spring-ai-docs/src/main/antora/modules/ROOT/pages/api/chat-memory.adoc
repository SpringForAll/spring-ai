...
`ChatMemory` 抽象旨在管理_聊天内存_。它允许您存储和检索与当前对话上下文相关的消息。但是，它不适合存储_聊天历史_。如果您需要维护所有交换消息的完整记录，您应该考虑使用不同的方法，例如依赖 Spring Data 来高效存储和检索完整的聊天历史。
...
| `spring.ai.chat.memory.repository.jdbc.platform` | 如果在初始化脚本中使用了 @@platform@@ 占位符，则使用的平台。 | _自动检测_
|===
...
警告：目前，在执行工具调用时与大型语言模型交换的中间消息不会存储在内存中。这是当前实现的限制，将在未来的版本中解决。如果您需要存储这些消息，请参阅 xref:api/tools.adoc#_user_controlled_tool_execution[用户控制的工具执行] 的说明。
...
注意：这里提供的 `PromptTemplate` 自定义 advisor 如何将检索到的内存与系统消息合并。这与在 `ChatClient` 本身上配置 `TemplateRenderer`（使用 `.templateRenderer()`）不同，后者影响 advisor 运行之前初始用户/系统提示内容的渲染。有关客户端级模板渲染的更多详细信息，请参阅 xref:api/chatclient.adoc#_prompt_templates[ChatClient 提示模板]。
