...
在向量数据库中，查询与传统关系型数据库不同。
它们不是进行精确匹配，而是执行相似性搜索。
当给定一个向量作为查询时，向量数据库会返回与查询向量"相似"的向量。
关于这种相似性如何在高层次上计算的更多细节，请参见 xref:api/vectordbs/understand-vectordbs.adoc#vectordbs-similarity[向量相似性]。

向量数据库用于将你的数据与 AI 模型集成。
使用它们的第一步是将数据加载到向量数据库中。
然后，当用户查询要发送到 AI 模型时，首先检索一组相似的文档。
这些文档随后作为用户问题的上下文，与用户的查询一起发送到 AI 模型。
这种技术被称为 xref:concepts.adoc#concept-rag[检索增强生成 (RAG)]。
...
要将数据插入向量数据库，需要将其封装在 `Document` 对象中。
`Document` 类封装了来自数据源（如 PDF 或 Word 文档）的内容，并包含以字符串形式表示的文本。
它还包含以键值对形式存储的元数据，包括文件名等详细信息。

在插入向量数据库时，文本内容使用嵌入模型转换为数值数组或 `float[]`，称为向量嵌入。嵌入模型，如 https://en.wikipedia.org/wiki/Word2vec[Word2Vec]、https://en.wikipedia.org/wiki/GloVe_(machine_learning)[GLoVE] 和 https://en.wikipedia.org/wiki/BERT_(language_model)[BERT]，或 OpenAI 的 `text-embedding-ada-002`，用于将单词、句子或段落转换为这些向量嵌入。
...
一些向量存储需要在使用前初始化其后端模式。
默认情况下不会为你初始化。
你必须通过传递适当的构造函数参数布尔值来选择加入，或者如果使用 Spring Boot，在 `application.properties` 或 `application.yml` 中将适当的 `initialize-schema` 属性设置为 `true`。
查看你使用的向量存储的文档以获取特定的属性名称。

== 批处理策略

在使用向量存储时，通常需要嵌入大量文档。
虽然一次性嵌入所有文档似乎很简单，但这种方法可能会导致问题。
嵌入模型将文本作为令牌处理，并且有最大令牌限制，通常称为上下文窗口大小。
这个限制限制了可以在单个嵌入请求中处理的文本量。
尝试在一次调用中嵌入太多令牌可能会导致错误或截断的嵌入。

为了解决这个令牌限制，Spring AI 实现了批处理策略。
这种方法将大型文档集分解成适合嵌入模型最大上下文窗口的较小批次。
批处理不仅解决了令牌限制问题，还可以提高性能并更有效地使用 API 速率限制。
...
此策略基于文档的令牌计数进行批处理，确保每个批次不超过计算的最大输入令牌计数。

`TokenCountBatchingStrategy` 的主要特点：

1. 使用 https://platform.openai.com/docs/guides/embeddings/embedding-models[OpenAI 的最大输入令牌计数]（8191）作为默认上限。
...
以下是使用 Vertex AI 的自动截断和自定义 `BatchingStrategy` 的配置示例，然后在 PgVectorStore 中使用它们：
...
虽然 `TokenCountBatchingStrategy` 提供了强大的默认实现，但你可以自定义批处理策略以适应特定需求。
...
注意：Spring AI 支持的向量存储配置为使用默认的 `TokenCountBatchingStrategy`。
SAP Hana 向量存储目前未配置批处理。

== VectorStore 实现

以下是 `VectorStore` 接口的可用实现：

* xref:api/vectordbs/azure.adoc[Azure Vector Search] - https://learn.microsoft.com/en-us/azure/search/vector-search-overview[Azure] 向量存储。
* xref:api/vectordbs/apache-cassandra.adoc[Apache Cassandra] - https://cassandra.apache.org/doc/latest/cassandra/vector-search/overview.html[Apache Cassandra] 向量存储。
* xref:api/vectordbs/chroma.adoc[Chroma Vector Store] - https://www.trychroma.com/[Chroma] 向量存储。
* xref:api/vectordbs/elasticsearch.adoc[Elasticsearch Vector Store] - https://www.elastic.co/[Elasticsearch] 向量存储。
* xref:api/vectordbs/gemfire.adoc[GemFire Vector Store] - https://tanzu.vmware.com/content/blog/vmware-gemfire-vector-database-extension[GemFire] 向量存储。
* xref:api/vectordbs/mariadb.adoc[MariaDB Vector Store] - https://mariadb.com/[MariaDB] 向量存储。
* xref:api/vectordbs/milvus.adoc[Milvus Vector Store] - https://milvus.io/[Milvus] 向量存储。
* xref:api/vectordbs/mongodb.adoc[MongoDB Atlas Vector Store] - https://www.mongodb.com/atlas/database[MongoDB Atlas] 向量存储。
* xref:api/vectordbs/neo4j.adoc[Neo4j Vector Store] - https://neo4j.com/[Neo4j] 向量存储。
* xref:api/vectordbs/opensearch.adoc[OpenSearch Vector Store] - https://opensearch.org/platform/search/vector-database.html[OpenSearch] 向量存储。
* xref:api/vectordbs/oracle.adoc[Oracle Vector Store] - https://docs.oracle.com/en/database/oracle/oracle-database/23/vecse/overview-ai-vector-search.html[Oracle Database] 向量存储。
* xref:api/vectordbs/pgvector.adoc[PgVector Store] - https://github.com/pgvector/pgvector[PostgreSQL/PGVector] 向量存储。
* xref:api/vectordbs/pinecone.adoc[Pinecone Vector Store] - https://www.pinecone.io/[PineCone] 向量存储。
* xref:api/vectordbs/qdrant.adoc[Qdrant Vector Store] - https://www.qdrant.tech/[Qdrant] 向量存储。
* xref:api/vectordbs/redis.adoc[Redis Vector Store] - https://redis.io/[Redis] 向量存储。
* xref:api/vectordbs/hana.adoc[SAP Hana Vector Store] - https://news.sap.com/2024/04/sap-hana-cloud-vector-engine-ai-with-business-context/[SAP HANA] 向量存储。
* xref:api/vectordbs/typesense.adoc[Typesense Vector Store] - https://typesense.org/docs/0.24.0/api/vector-search.html[Typesense] 向量存储。
* xref:api/vectordbs/weaviate.adoc[Weaviate Vector Store] - https://weaviate.io/[Weaviate] 向量存储。
* link:https://github.com/spring-projects/spring-ai/blob/main/spring-ai-vector-store/src/main/java/org/springframework/ai/vectorstore/SimpleVectorStore.java[SimpleVectorStore] - 一个简单的持久化向量存储实现，适合教育目的。

未来版本可能会支持更多实现。

如果你有一个需要 Spring AI 支持的向量数据库，请在 GitHub 上提出 issue，或者更好的是，提交一个带有实现的 pull request。

每个 `VectorStore` 实现的信息可以在本章的各个小节中找到。
...
给定一个表示 JSON 文件的源文件的 `String` 引用，其中包含我们要加载到向量数据库中的数据，我们使用 Spring AI 的 `JsonReader` 来加载 JSON 中的特定字段，将它们分割成小块，然后将这些小块传递给向量存储实现。
`VectorStore` 实现计算嵌入并将 JSON 和嵌入存储在向量数据库中：
...
稍后，当用户问题传递到 AI 模型时，会进行相似性搜索以检索相似文档，然后将这些文档"填充"到提示中作为用户问题的上下文。
...
你可以使用 `FilterExpressionBuilder` 创建一个 `Filter.Expression` 实例，它提供了一个流畅的 API。
一个简单的例子如下：
...
此方法删除 ID 与提供的列表匹配的所有文档。
如果列表中的任何 ID 在存储中不存在，它将被忽略。
...
xref:api/vectordbs/understand-vectordbs.adoc[理解向量]
