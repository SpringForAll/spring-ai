...
* **同步服务器** - 使用 `McpSyncServer` 实现的默认服务器类型。
它设计用于应用程序中的简单请求-响应模式。
要启用此服务器类型，在配置中设置 `spring.ai.mcp.server.type=SYNC`。
激活时，它会自动处理同步工具规范的配置。

* **异步服务器** - 异步服务器实现使用 `McpAsyncServer`，针对非阻塞操作进行了优化。
要启用此服务器类型，使用 `spring.ai.mcp.server.type=ASYNC` 配置您的应用程序。
此服务器类型自动设置具有内置 Project Reactor 支持的异步工具规范。
...
MCP 服务器启动器允许服务器向客户端暴露工具、资源和提示。
它自动将注册为 Spring bean 的自定义功能处理器转换为基于服务器类型的同步/异步规范：

=== link:https://spec.modelcontextprotocol.io/specification/2024-11-05/server/tools/[工具]
允许服务器暴露可由语言模型调用的工具。MCP 服务器启动器提供：

* 变更通知支持
* xref:api/tools.adoc[Spring AI 工具] 根据服务器类型自动转换为同步/异步规范
...
支持 xref:api/tools.adoc#_tool_context[工具上下文]，允许将上下文信息传递给工具调用。它在 `exchange` 键下包含一个 `McpSyncServerExchange` 实例，可通过 `McpToolUtils.getMcpExchange(toolContext)` 访问。请参阅此 https://github.com/spring-projects/spring-ai-examples/blob/3fab8483b8deddc241b1e16b8b049616604b7767/model-context-protocol/sampling/mcp-weather-webmvc-server/src/main/java/org/springframework/ai/mcp/sample/server/WeatherService.java#L59-L126[示例]，演示了 `exchange.loggingNotification(...)` 和 `exchange.createMessage(...)`。

=== link:https://spec.modelcontextprotocol.io/specification/2024-11-05/server/resources/[资源管理]
...
=== link:https://spec.modelcontextprotocol.io/specification/2024-11-05/server/prompts/[提示管理]
...
=== link:https://spec.modelcontextprotocol.io/specification/2024-11-05/server/completions/[完成管理]
...
=== link:https://spec.modelcontextprotocol.io/specification/2024-11-05/client/roots/#root-list-changes[根变更消费者]
...
您可以有多个产生 ToolCallbacks 的 bean。自动配置将合并它们。

== 示例应用程序
* link:https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/weather/starter-webflux-server[天气服务器（WebFlux）] - 具有 WebFlux 传输的 Spring AI MCP 服务器启动器。
* link:https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/weather/starter-stdio-server[天气服务器（STDIO）] - 具有 STDIO 传输的 Spring AI MCP 服务器启动器。
* link:https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/weather/manual-webflux-server[天气服务器手动配置] - 不使用自动配置但使用 Java SDK 手动配置服务器的 Spring AI MCP 服务器启动器。

== 其他资源

* link:https://docs.spring.io/spring-ai/reference/[Spring AI 文档]
* link:https://modelcontextprotocol.github.io/specification/[模型上下文协议规范]
* link:https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.developing-auto-configuration[Spring Boot 自动配置]
