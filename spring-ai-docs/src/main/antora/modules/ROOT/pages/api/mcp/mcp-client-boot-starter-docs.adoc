...
对于生产部署，我们建议使用基于 WebFlux 的 SSE 连接，即 `spring-ai-starter-mcp-client-webflux`。
...
|`type`
|客户端类型（SYNC 或 ASYNC）。所有客户端必须是同步或异步的；不支持混合使用
|`SYNC`
...
或者，您可以使用外部 JSON 文件配置 stdio 连接，使用 link:https://modelcontextprotocol.io/quickstart/user[Claude Desktop 格式]：
...
* *请求配置* - 设置自定义请求超时
* link:https://spec.modelcontextprotocol.io/specification/2024-11-05/client/sampling/[*自定义采样处理器*] - 服务器通过客户端从 LLM 请求 LLM 采样（`completions` 或 `generations`）的标准化方式。此流程允许客户端保持对模型访问、选择和权限的控制，同时使服务器能够利用 AI 功能 — 无需服务器 API 密钥。
* link:https://spec.modelcontextprotocol.io/specification/2024-11-05/client/roots/[*文件系统（根）访问*] - 客户端向服务器公开文件系统 `roots` 的标准化方式。
Roots 定义了服务器可以在文件系统中操作的边界，允许它们了解可以访问哪些目录和文件。
服务器可以从支持根变更的客户端请求根列表，并在该列表更改时接收通知。
* *事件处理器* - 当发生特定服务器事件时通知客户端的处理器：
  - 工具变更通知 - 当可用服务器工具列表更改时
  - 资源变更通知 - 当可用服务器资源列表更改时
  - 提示变更通知 - 当可用服务器提示列表更改时
* link:https://spec.modelcontextprotocol.io/specification/2024-11-05/server/utilities/logging/[*日志处理器*] - 服务器向客户端发送结构化日志消息的标准化方式。
...
- link:https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/web-search/brave-chatbot[Brave Web 搜索聊天机器人] - 使用模型上下文协议与 Web 搜索服务器交互的聊天机器人。
- link:https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/client-starter/starter-default-client[默认 MCP 客户端启动器] - 使用默认 `spring-ai-starter-mcp-client` MCP 客户端启动器的简单示例。
- link:https://github.com/spring-projects/spring-ai-examples/tree/main/model-context-protocol/client-starter/starter-webflux-client[WebFlux MCP 客户端启动器] - 使用 `spring-ai-starter-mcp-client-webflux` MCP 客户端启动器的简单示例。
